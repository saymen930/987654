...
from InflexMusic.mongo.readable_time import get_readable_time
from InflexMusic.mongo.afkdb import add_afk, is_afk, remove_afk
...
async def active_afk(_, message: Message):
        ...
        try:
            ...
            if afktype == "text":
                send = await message.reply_text(
                    f"**{message.from_user.first_name}** İndi onlayndır və {seenago} müddətində yox idi.",
            ...
            if afktype == "animation":
                if str(reasonafk) == "None":
                    send = await message.reply_animation(
                        data,
                        caption=f"**{message.from_user.first_name}** İndi onlayndır və {seenago} müddətində yox idi.",
                    )
                else:
                    send = await message.reply_animation(
                        data,
            ...
            send = await message.reply_text(
                f"**{message.from_user.first_name}** İndi onlayndır.",
                disable_web_page_preview=True,
            )
    ...
    await message.reply_text(f"{message.from_user.first_name} indi AFK vəziyyətindədir!")
...
async def chat_watcher_func(_, message):
    ...
    if verifier:
        ...
        try:
            ...
            seenago = get_readable_time((int(time.time() - timeafk)))
            if afktype == "text":
                msg += f"**{user_name[:25]}** İndi onlayndır və {seenago} müddətində yox idi.\n\n"
            if afktype == "text_reason":
                msg += f"**{user_name[:25]}** İndi onlayndır və {seenago} müddətində yox idi.\n\nSəbəb: `{reasonafk}`\n\n"
            if afktype == "animation":
                if str(reasonafk) == "None":
                    send = await message.reply_animation(
                        data,
                        caption=f"**{user_name[:25]}** İndi onlayndır və {seenago} müddətində yox idi.\n\n",
                    )
                else:
                    send = await message.reply_animation(
                        data,
            ...
            msg += f"**{user_name[:25]}** İndi onlayndır.\n\n"
